...
;MOMENT CALCULATIONS
cdpThirdM=make_array(n_elements(pmb))
...
for m=0, n_elements(pmb)-1 do begin
  ...
  for j=0,n_elements(cdpdbins[*,0,0])-1 do begin
    x=[x,(diam[j])^3.*(cdpdbins[j,0,m])]
  endfor
    y=(total(x)/total(cdpdbins[*,0,m]))^(1./3.)
    cdpThirdM[m]=y
endfor
...
g  = {as:as, pmb:pmb, time:time, timeForm:timeForm, avroll:avroll, avpitch:avpitch, $
  ...
  signalTot:signalTot,signalLiq:signalLiq,cdpdbins:cdpdbins,cdpThirdM:cdpThirdM}
