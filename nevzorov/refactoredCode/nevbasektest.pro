function nevbasektest, flightDay, airspeedType, level
  ...
  if flightDay eq 'copecal900' then begin
    flightString='08-03-13'
    aStart=where(timeForm eq 114652)
    aEnd=where(timeForm eq 120435)
  endif
  if flightDay eq 'copecal600' then begin
    flightString='08-03-13'
    aStart=where(timeForm eq 112528)
    aEnd=where(timeForm eq 113859)
  endif
  if flightDay eq 'copecal400' then begin
    flightString='08-03-13'
    aStart=where(timeForm eq 104740)
    aEnd=where(timeForm eq 110400)
  endif



  if flightDay eq 'larcal900' then begin
    flightString='08-03-13'
    aStart=where(timeForm eq 171200)
    aEnd=where(timeForm eq 172200)
  endif
  if flightDay eq 'larcal600' then begin
    flightString='08-03-13'
    aStart=where(timeForm eq 175000)
    aEnd=where(timeForm eq 180500)
  endif
  if flightDay eq 'larcal400' then begin
    flightString='08-03-13'
    aStart=where(timeForm eq 181600)
    aEnd=where(timeForm eq 184400)
  endif
  ...
  kliqcalc=(vlwccol*ilwccol)/(vlwcref*ilwcref)
  ...
  g  = {as:as, pmb:pmb, time:time, timeForm:timeForm, avroll:avroll, avpitch:avpitch, $
    pLiq:pLiq, lwc:lwc, lwcPresCor:lwcPresCor, $
    clearAir:clearAir, levelClearAir:levelClearAir, linPresCor:linPresCor, $
    flightString:flightString, kLiq:kLiq, clearAirLargeErr:clearAirLargeErr, $
    clearAirLargeErrex:clearAirLargeErrex, levelClearAirLargeErrex:levelClearAirLargeErrex, $
    aiasMs:aiasMs, tas:tas, baselineClimbTimes:baselineClimbTimes,baselineClimbTimesNon:baselineClimbTimesNon, $
    linPresCorSteepClimb:linPresCorSteepClimb, baselineClimbTimesNonLevel:baselineClimbTimesNonLevel, $
    vlwccol:vlwccol, ilwccol:ilwccol, cdpconc_NRB:cdpconc_NRB, trf:trf, kliqcalc:kliqcalc,$
    lwc100:lwc100, cdpdbar_NRB:cdpdbar_NRB}
  ;g.as=as
  ;g.pmb=pmb
